  ...
  if (!isAuthorized) {
    return (
      <div className="flex items-center justify-center min-h-[60vh] text-right" dir="rtl">
        <div className="bg-white p-12 rounded-[3rem] shadow-2xl border-4 border-indigo-600 max-w-md w-full animate-in zoom-in duration-300">
          <div className="w-20 h-20 bg-indigo-600 rounded-3xl flex items-center justify-center text-white mx-auto mb-6 shadow-xl">
            <Lock size={40} />
          </div>
          <h2 className="text-2xl font-black text-center mb-2">منطقة المدير العام</h2>
          <p className="text-slate-400 text-center text-xs mb-8">يرجى إدخال المفتاح السري الماستر للوصول للسحابة</p>
          
          <form onSubmit={handleMasterLogin} className="space-y-4">
            <input 
              type="password"
              className="w-full p-5 bg-slate-50 border-2 border-transparent focus:border-indigo-600 rounded-2xl font-black text-center outline-none transition"
              placeholder="••••••••••••"
              value={masterKey}
    ...
    <div className="p-8 space-y-8 text-right animate-in fade-in duration-500" dir="rtl">
      ...
      {data && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {/* معلومات الرخصة */}
          <div className="lg:col-span-3 bg-indigo-50 p-6 rounded-3xl border-2 border-indigo-100 flex items-center justify-between">
             <div className="flex items-center gap-4">
                ...
                <div>
                   ...
                   <p className="font-black text-indigo-900">{data.info.id}</p>
                </div>
             </div>
             <div className="text-left">
                <p className="text-[10px] font-black text-indigo-400 uppercase">اسم الجهاز المرتبط</p>
                <p className="font-black text-indigo-900">{data.info.device_name || 'غير معروف'}</p>
             </div>
          </div>

          {/* قائمة الموظفين */}
          <div className="lg:col-span-1 bg-white p-8 rounded-[2.5rem] shadow-xl border">
            ...
            <div className="space-y-4 max-h-[600px] overflow-y-auto custom-scrollbar">
              {data.employees.map((emp: any) => (
                <div key={emp.id} className="p-4 bg-slate-50 rounded-2xl flex justify-between items-center border border-transparent hover:border-indigo-200 transition">
          ...
          <div className="lg:col-span-2 bg-white p-8 rounded-[2.5rem] shadow-xl border">
            <h3 className="text-xl font-black mb-6 flex items-center gap-2 text-rose-600"><History /> تتبع النشاط الحي (Audit Logs)</h3>
            <div className="space-y-4 max-h-[600px] overflow-y-auto custom-scrollbar">
              {data.logs.map((log: any) => (
                <div key={log.id} className="p-5 border-r-8 border-rose-500 bg-rose-50/30 rounded-2xl">
                  ...
                  <p className="font-black text-sm text-slate-800 mb-2">تعديل على جدول: <span className="text-indigo-600">{log.target_table}</span></p>
                  <div className="bg-white/60 p-4 rounded-xl text-[11px] font-mono overflow-x-auto">
      ...
      {/* تنبيه أمني */}
      <div className="bg-amber-50 p-6 rounded-3xl border-2 border-amber-100 flex items-center gap-4 text-amber-800 no-print">
         ...
         <button onClick={() => setIsAuthorized(false)} className="mr-auto bg-amber-200 px-6 py-2 rounded-xl font-black text-xs hover:bg-amber-300 transition">خروج آمن</button>
      </div>
    </div>
  );
};
