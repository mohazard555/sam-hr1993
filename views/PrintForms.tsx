...
import { Printer, FileText, ClipboardList, AlertTriangle, CheckCircle2, Calendar, UserCheck } from 'lucide-react';
...
const PrintForms: React.FC<Props> = ({ employees, settings, onPrint }) => {
  ...
  const formConfigs = {
    ...
    permission: { 
      ...
      placeholder: 'يمنح الموظف المذكور أدناه إذن خروج مؤقت من مقر العمل اليوم في تمام الساعة ... وذلك للضرورة القصوى، على أن يعود في تمام الساعة ...' 
    },
    warning: { 
      title: 'كتاب لفت نظر إداري', 
      icon: FileText, 
      color: 'text-amber-600', 
      placeholder: 'بالإشارة إلى السجلات الإدارية، فقد تبيّن لنا قيامكم بمخالفة (تأخر متكرر/عدم الالتزام بالمهام) وذلك بتاريخ ...\nوعليه يتم لفت نظركم بضرورة التقيد بالأنظمة والتعليمات تحت طائلة المسؤولية.' 
    },
    penalty: { 
      title: 'قرار عقوبة إدارية', 
      icon: AlertTriangle, 
      color: 'text-rose-600', 
      placeholder: 'قررت إدارة المؤسسة بناءً على التوصية الإدارية رقم (...)، فرض عقوبة (خصم يوم من الراتب) على الموظف المذكور وذلك لمخالفته تعليمات العمل المباشرة بتاريخ ...' 
    },
  };
      ...
      <div className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] shadow-2xl border dark:border-slate-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
           <div>
              ...
              <select className="w-full p-4 bg-slate-50 dark:bg-slate-800 border-2 rounded-2xl font-black outline-none focus:border-indigo-600 transition" value={selectedEmp} onChange={e => setSelectedEmp(e.target.value)}>
              ...
              <div className="grid grid-cols-2 gap-3">
                 {(Object.keys(formConfigs) as Array<keyof typeof formConfigs>).map(type => (
                   <button key={type} onClick={() => { setFormType(type); setDetails(formConfigs[type].placeholder); }} className={`p-4 rounded-2xl border-2 transition-all flex flex-col items-center gap-2 ${formType === type ? 'border-indigo-600 bg-indigo-50 dark:bg-indigo-900/20' : 'border-slate-100 dark:border-slate-800'}`}>
                      ...
                      <span className={`text-[10px] font-black ${formType === type ? 'text-indigo-700' : 'text-slate-500'}`}>{formConfigs[type].title}</span>
                   </button>
                 ))}
              </div>
           </div>
        ...
        <div className="space-y-6">
           <div>
              ...
              <textarea className="w-full p-8 bg-slate-50 dark:bg-slate-800 border-2 rounded-3xl font-bold h-64 outline-none transition-all resize-none text-lg leading-relaxed" placeholder="أدخل نص الخطاب هنا..." value={details} onChange={e => setDetails(e.target.value)}></textarea>
           </div>
