...
export function GenericModule<T extends { id: string; employeeId: string; date?: string; startDate?: string; endDate?: string; amount?: number; type?: string; remainingAmount?: number; isPaid?: boolean }>({ 
  title, lang, employees, items, onSave, onDelete, onPrint, renderForm, renderRow, tableHeaders, initialData 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.employeeId) return alert('اختر موظف');
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] shadow-xl border dark:border-slate-800 no-print">
        ...
        <div className="flex gap-2">
          <button onClick={() => { setFormData(initialData); setShowModal(true); }} className="bg-indigo-600 text-white px-6 py-3 rounded-2xl font-black flex items-center gap-2"><Plus size={20} /> إضافة جديد</button>
          ...
          {onPrint && <button onClick={onPrint} className="bg-slate-900 text-white px-6 py-3 rounded-2xl font-black flex items-center gap-2"><Printer size={20} /> طباعة</button>}
        </div>
      </div>

      <div className="relative no-print"><Search className="absolute right-4 top-3.5 text-slate-400" size={18}/><input className="w-full pr-12 pl-4 py-3 bg-white dark:bg-slate-900 border rounded-2xl font-black" placeholder="بحث باسم الموظف..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} /></div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-xl z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-[3rem] shadow-2xl w-full max-w-2xl border dark:border-slate-800">
            <div className="p-8 bg-slate-50 dark:bg-slate-800 border-b flex justify-between items-center">
